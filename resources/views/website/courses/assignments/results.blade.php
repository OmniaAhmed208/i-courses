                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="quiz-content text-center">
                            @php
                                $percentage = ($assignment->total_mark > 0) ? number_format($assignment_attempt->mark  / $assignment->total_mark * 100, 2) : 0;
                            @endphp
                            ...
                            <p class="lead font-weight-regular font-size-18 text-color-rgba mb-0 pb-2">
                                @lang('site.your_scores'): {{ $assignment_attempt->mark }}/{{ $assignment->total_mark }}
                            </p>
                        </div>
                    </div><!-- end col-lg-12 -->
                </div><!-- end row -->
                        ...
                        <div class="quiz-action-content d-flex align-items-center justify-content-between">
                            <ul class="quiz-nav d-flex align-items-center">
                                ...
                                <li>
                                    ...
                                    {{ $assignment_attempt->mark }} / {{ $assignment->total_mark }} @lang('site.scores')
                                </li>
                                    ...
                                    <span
                                        class="quiz-count icon-element icon--element bg-color-1 text-white mr-2">{{ $index + 1 }}</span>
                                            <div
                                                ...
                                                @if($assignment_attempt->getAnswerById($question->id))
                                                    <span
                                                        class="font-size-14 {{ !$assignment_attempt->is_final_mark && $question->type == 'essay' ? 'text-danger' : '' }}">
                                                    {{ !$assignment_attempt->is_final_mark && $question->type == 'essay' ? __('site.question_didnt_marked') : '' }} ({{ $assignment_attempt->getAnswerById($question->id)->mark }}/{{ $question->mark }})
                                                    </span>
                                                @else
                                            ...
                                            <ul class="quiz-result-list pt-4 pl-3 mb-4 {{ app()->getLocale() == 'ar' ? 'text-left' : '' }}">
                                                @foreach($question->choices as $key => $choice)
                                                    <li class="primary-color mb-2">
                                                        ...
                                                        @elseif(!$choice->correct &&  $assignment_attempt->getAnswerById($question->id) && $assignment_attempt->getAnswerById($question->id)->answer == $choice->title)
                                        ...
                                        @elseif($question->type == 'true_false')
                                            <ul class="quiz-result-list pt-4 pl-3 mb-4 {{ app()->getLocale() == 'ar' ? 'text-left' : '' }}">
                                                <li class="primary-color mb-2">
                                                    ...
                                                    @elseif($assignment_attempt->getAnswerById($question->id)->answer == "true" && $question->choices->correct_val == "false")
                                                        <span class="icon-element icon--element icon-error mr-2">
                                                            A
                                                        </span>
                                                    @else
